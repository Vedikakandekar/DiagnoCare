...
<div style="margin-left:30%;padding:1px 16px;height:1000px;">
    ...
    <table border="1">
        ...
        $result=mysqli_query($con,"select * from path_appointments where user_id='$userid' "); //added when error occured
        ...
        <tr>
            ...
            <table border="1">
                <?php foreach($result as $row){
                    ...
                    <td>
                        <?php if ($row["test_progress"]=="Report is Ready"){?>
                            <a href="DownloadReport.php?test_id=<?=$row['test_id']?>">Download Report</a>
                        <?php }else{ echo $row["test_progress"];}?>
                    </td>
